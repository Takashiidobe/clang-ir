...
use crate::ast::{Attribute, Block, Module, Operation, Region, SourceLocation, Type};
...
impl<'a> Parser<'a> {
    ...
    /// Consumes and returns a trailing `loc(...)`, or leaves the parser
    /// untouched and returns `None` if the current token isn't `loc`.
    fn try_parse_loc_suffix(&mut self) -> Result<Option<SourceLocation>> {
        ...
        let location = self.parse_source_location_body(&[Tok::RParen])?;
        self.expect(Tok::RParen)?;
        Ok(Some(location))
    }

    fn parse_source_location_body(&mut self, terminators: &[Tok]) -> Result<SourceLocation> {
        if self.tok_is_ident("unknown") {
            self.bump();
            return Ok(SourceLocation::Unknown);
        }

        if self.tok_is_ident("fused") {
            self.bump();
            self.expect(Tok::LBracket)?;
            let mut locations = Vec::new();
            if !self.at(&Tok::RBracket) {
                loop {
                    locations.push(self.parse_source_location_body(&[Tok::Comma, Tok::RBracket])?);
                    if !self.eat(&Tok::Comma) {
                        break;
                    }
                }
            ...
            self.expect(Tok::RBracket)?;
            return Ok(SourceLocation::Fused(locations));
        }

        if self.starts_file_line_col_loc() {
            let file = self.expect_str()?;
            self.expect(Tok::Colon)?;
            let line = self.parse_location_u32()?;
            self.expect(Tok::Colon)?;
            let column = self.parse_location_u32()?;
    ...
    fn tok_is_ident(&self, expected: &str) -> bool {
        matches!(self.tok(), Tok::Ident(s) if s == expected)
    }

    fn starts_file_line_col_loc(&self) -> bool {
        let pos = self.pos;
        matches!(self.toks.get(pos).map(|t| &t.tok), Some(Tok::Str(_)))
            && matches!(self.toks.get(pos + 1).map(|t| &t.tok), Some(Tok::Colon))
            && matches!(self.toks.get(pos + 2).map(|t| &t.tok), Some(Tok::Number(_)))
            && matches!(self.toks.get(pos + 3).map(|t| &t.tok), Some(Tok::Colon))
            && matches!(self.toks.get(pos + 4).map(|t| &t.tok), Some(Tok::Number(_)))
    }

    fn capture_location_body(&mut self, terminators: &[Tok]) -> String {
        let start = self.cur().pos;
        let mut depth = 0usize;
        loop {
            if depth == 0 && terminators.contains(self.tok()) {
                return self.src[start..self.cur().pos].to_string();
            }
            match self.tok() {
                Tok::Less | Tok::LParen | Tok::LBrace | Tok::LBracket => depth += 1,
                Tok::Greater | Tok::RParen | Tok::RBrace | Tok::RBracket if depth > 0 => {
                    depth -= 1;
                }
                Tok::Eof => {
                    return self.src[start..self.cur().pos].to_string();
    ...
    fn parse_location_u32(&mut self) -> Result<u32> {
        self.expect_number()?
            .parse()
            .map_err(|_| self.err("invalid location line or column"))
    }
...
mod tests {
    ...
    #[test]
    fn parses_file_line_column_location() {
        let module =
            parse_generic_module(r#""cir.return"() : () -> () loc("/tmp/foo.c":3:9)"#).unwrap();
        assert_eq!(
            module.ops[0].loc,
            Some(SourceLocation::File {
                file: "/tmp/foo.c".to_string(),
                line: 3,
                column: 9,
            })
        );
    ...
    fn parses_unknown_location() {
        let module = parse_generic_module(r#""cir.return"() : () -> () loc(unknown)"#).unwrap();
        assert_eq!(module.ops[0].loc, Some(SourceLocation::Unknown));
    }

    #[test]
    fn parses_fused_locations() {
        let module = parse_generic_module(
            r#""cir.return"() : () -> () loc(fused["/tmp/a.c":1:1, "/tmp/b.c":2:2])"#,
        )
        .unwrap();
        assert_eq!(
            module.ops[0].loc,
            Some(SourceLocation::Fused(vec![
                SourceLocation::File {
